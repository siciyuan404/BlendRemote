...
/**
 * 应用自更新检查器(参考 MeowMic UpdateChecker)
 *
 * 通过 GitHub Releases API 检查最新版本,下载 APK 并调起系统安装器。
 * - GitHub API: GET /repos/{owner}/{repo}/releases/latest
 * - 找 tag_name + 名为 blendremote-{tag}.apk 的 asset
 */
class UpdateChecker(private val context: Context) {

    companion object {
        private const val TAG = "BlendRemote/Update"
        private const val OWNER = "siciyuan404"
        private const val REPO = "BlendRemote"
        // APK asset 命名规则:blendremote-v{tag}.apk
        private const val APK_PREFIX = "blendremote-"
    ...
    suspend fun checkLatest(): UpdateState = withContext(Dispatchers.IO) {
        try {
            ...
            val tagName = json.optString("tag_name", "")  // v0.1.4
    ...
    suspend fun downloadApk(url: String, onProgress: (Int) -> Unit): String = withContext(Dispatchers.IO) {
        ...
        val apkFile = File(dir, "blendremote-update.apk")
