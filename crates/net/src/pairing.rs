//! 配对机制(参考 MeowMic / Moonlight + Sunshine 的 PIN 配对)
//!
//! 设计:
//! - 服务端首次启动生成 Ed25519 密钥对,持久化到本地 pairing.json
//! - 启动时若无已配对客户端,生成 6 位 PIN(Blender 插件 N 面板展示)
...
//!
//! 持久化路径(服务端):
//! - Windows: %APPDATA%/blendremote/pairing.json
//! - Linux/macOS: ~/.config/blendremote/pairing.json
...
impl PairingManager {
    /// 加载或创建配对状态
    ///
    /// - `state_dir`:状态文件所在目录(如 %APPDATA%/blendremote)
    ...
    /// 强制刷新 PIN(旧 PIN 失效,正在配对的设备需要重新输入)
    ...
    pub async fn verify_paired_hello(
        ...
        let verifying_key = VerifyingKey::from_bytes(
            client_pubkey
                .try_into()
                .map_err(|_| PairingError::Ed25519("VerifyingKey::from_bytes 长度不匹配".into()))?,
        )
...
mod tests {
    ...
    async fn test_pairing_flow() {
        let tmp_dir = std::env::temp_dir().join(format!("blendremote-test-{}", std::process::id()));
        ...
        let (client_pubkey2, hello_sig) =
            client_state.sign_paired_hello("TestClient1", hello_nonce).unwrap();
        server
    ...
    async fn test_bad_pin() {
        let tmp_dir = std::env::temp_dir().join(format!("blendremote-test-bad-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp_dir);
        std::fs::create_dir_all(&tmp_dir).unwrap();
